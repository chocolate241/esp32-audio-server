...
<body class="text-slate-200 font-sans p-4 md:p-6">
    <div class="max-w-6xl mx-auto space-y-6">
        <header class="flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-800 pb-4">
            <div>
                <h1 class="text-2xl font-bold text-cyan-400 flex items-center gap-2"> Hệ Thống Radar Âm Thanh & AI</h1>
    ...
    ws.on('message', async (message, isBinary) => {
        if (isBinary) {
            ...
            if (!recordingTimers.has(ws)) {
                audioBuffers.set(ws, []);
                console.log(' [Kích hoạt] Thu âm câu lệnh (Tối đa 5s)...');
...
async function processCommand(ws) {
    ...
    broadcastToMonitor({
        type: 'MONITOR_UPDATE',
        state: 'ĐANG XỬ LÝ LỆNH...',
        bufferLength: 0,
        log: 'Khóa luồng thu âm! Đang đóng gói dữ liệu và biên dịch qua Gemini AI...',
        logType: 'warn'
    });
